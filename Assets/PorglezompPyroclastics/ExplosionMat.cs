...
public class ExplosionMat : MonoBehaviour {
	...
	void Start () {
		gameObject.GetComponent<Renderer>().material = new Material(ExplosionMaterial);
	...
	void Update () {
		if (doUpdate) {
			Material rsm = gameObject.GetComponent<Renderer>().material;
	...
	void SetShaderKeywords () {
        ...
        gameObject.GetComponent<Renderer>().material.shaderKeywords = newKeywords.ToArray();
	}
}
