...
public class Car : MonoBehaviour
{
   ...
   // public CarDriveType carDriveType { get { return m_CarDriveType; } }
    ...
    public float frontBrakeTorque = 0; // Brake torque to apply for each front wheel when Braking
    ...
    public float MaxSpeed = 100;//car's Top speed//{ get { return m_Topspeed; } }
    ...
    private Transform wheelColliders_trans;
    ...
    public Wheel[] backWheels = new Wheel[4];
    public Wheel[] leftWheels = new Wheel[3];
    public Wheel[] rightWheels = new Wheel[3];

    public Steering steering;

    void Awake()
    {
        wheelColliders_trans = transform.FindChild("WheelColliders");
        ...
        if (wheels == null || wheels.Length == 0)
        {
            ...
            wheels[4] = buildWheel("wheelML", false, true);
            wheels[5] = buildWheel("wheelMR", false, true);
        }
    ...
    Wheel buildWheel(string wheelName, bool isFront, bool isMotor)
    {
        ...
        WheelCollider wheelCollider = wheelColliders_trans.FindChild(wheelName).GetComponent<WheelCollider>();// find a wheel colider with the same name
