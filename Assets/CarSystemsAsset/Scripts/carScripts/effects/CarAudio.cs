...
public class CarAudio : MonoBehaviour
{
    ...
    private bool m_StartedSound; // flag for knowing if we have started sounds
                                 //  private CarEffects m_CarEffects; // Reference to car we are controlling
    CarControl m_Car;
    private void StartSound()
    {
        // get the carcontroller ( this will not be null as we have require component)
        // m_CarEffects = GetComponent<CarEffects>();
    ...
    void Update()
    {
        ...
        camDist = (carCamera.transform.position - transform.position).sqrMagnitude;
        ...
        if (m_StartedSound)
        {
            ...
            float pitch = ULerp(lowPitchMin, lowPitchMax,
                m_Car.car.engine.currentRPM / m_Car.car.engine.maxRPM);
            ...
            else
            {
                ...
                float accFade = Mathf.Abs(m_Car.AccelInput);
                float decFade = 1 - accFade;

                // get the high fade value based on the cars revs
                // float highFade = Mathf.InverseLerp(0.2f, 0.8f, m_CarEffects.Revs);
                float highFade = Mathf.InverseLerp(0.2f, 0.8f, m_Car.car.engine.currentRPM
                    / m_Car.car.engine.maxRPM);
                ...
                m_LowAccel.volume = lowFade * accFade / 2;
                m_LowDecel.volume = lowFade * decFade / 2;
                m_HighAccel.volume = highFade * accFade / 2;
                m_HighDecel.volume = highFade * decFade / 2;
