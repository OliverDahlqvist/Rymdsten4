...
namespace DeepSky.Haze
{
    ...
    public class DS_HazeLightVolume : MonoBehaviour
    {
        ...
        {
            return isActiveAndEnabled & Vector3.Distance(cameraPos, m_Transform.position) < m_EndFade;
        }
        ...
        private int SetShaderPassAndMatrix(Transform cameraTransform, out Matrix4x4 worldMtx)
        {
            worldMtx = Matrix4x4.TRS(m_Transform.position, m_Transform.rotation, new Vector3(m_Light.range, m_Light.range, m_Light.range));
            ...
            if (m_Light.type == LightType.Spot)
            {
                ...
                Vector3 lvec = (cameraTransform.position - m_Transform.position).normalized;
                float LdotC = Vector3.Dot(lvec, m_Transform.forward);
                if (LdotC > cosTheta)
                {
        ...
        public void FillLightCommandBuffer(RenderTexture radianceTarget, Transform cameraTransform)
        {
            m_RenderCmd.SetGlobalTexture("_ShadowMapTexture", BuiltinRenderTextureType.CurrentActive);

            Matrix4x4 worldMtx;
            int shaderPass = SetShaderPassAndMatrix(cameraTransform, out worldMtx);

            m_RenderCmd.SetRenderTarget(radianceTarget);
            m_RenderCmd.DrawMesh(m_ProxyMesh, worldMtx, m_VolumeMaterial, 0, shaderPass);
        }

        public void AddLightRenderCommand(Transform cameraTransform, CommandBuffer cmd)
        {
            Matrix4x4 worldMtx;
            int shaderPass = SetShaderPassAndMatrix(cameraTransform, out worldMtx);
        ...
        public void SetupMaterialPerFrame(Matrix4x4 viewProjMtx, Matrix4x4 viewMtx, Transform cameraTransform, Vector2 samplingOffset)
        {
            ...
            float fadeFactor = 1.0f - Mathf.Clamp01((Vector3.Distance(cameraTransform.position, m_Transform.position) - m_StartFade) / (m_EndFade - m_StartFade));

            m_VolumeMaterial.SetVector("_DS_HazeSamplingParams", new Vector4(samplingOffset.x, samplingOffset.y, m_DensityTextureContrast, 0));
            ...
            m_VolumeMaterial.SetVector("_DS_HazeLightVolumeParams0", new Vector4(m_Transform.position.x, m_Transform.position.y, m_Transform.position.z, m_Light.range));

            Matrix4x4 worldMtx = Matrix4x4.TRS(m_Transform.position, m_Transform.rotation, new Vector3(m_Light.range, m_Light.range, m_Light.range));
            ...
            if (m_Light.type == LightType.Point)
            {
                ...
                if (m_Light.cookie)
                {
                    ...
                    m_VolumeMaterial.SetMatrix("_DS_Haze_WorldToCookie", m_Transform.worldToLocalMatrix);
            ...
            else if (m_Light.type == LightType.Spot)
            {
                ...
                if (shadows)
                {
                    ...
                    Matrix4x4 lightView = Matrix4x4.TRS(m_Transform.position, m_Transform.rotation, Vector3.one).inverse;
                ...
                float cosTheta = Mathf.Cos((m_Light.spotAngle / 2.0f) * Mathf.Deg2Rad);
                Vector3 planeC = m_Transform.position + m_Transform.forward * m_Light.range;
                float planeD = -Vector3.Dot(planeC, m_Transform.forward);

                m_VolumeMaterial.SetVector("_DS_HazeLightVolumeParams1", new Vector4(m_Transform.forward.x, m_Transform.forward.y, m_Transform.forward.z, 1.0f));
